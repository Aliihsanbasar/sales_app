...
public class ProductModel {
    ...
    public DefaultTableModel productTable() {
        ...
        for (Product item : prls) {
            Object[] row = {item.getPrid(), item.getPrcid(), item.getPrtitle(), item.getPrprice(), item.getCurType(), item.getPrbarcode(),
    ...
    public int productUpdate(int prid, int prcid, String prtitle, double prprice, String prbarcode, String prshelf, int prstock, String prnote, int cType) {
        int statu = -1;
        try {
            DB db = new DB();
            String query = "update product set prcid = ?,  prtitle = ?, prprice = ?, prbarcode = ?, prshelf = ?, prstock = ?, prnote = ?, curType = ? where prid = ?";
            PreparedStatement pre = db.fncPre(query);
            pre.setInt(1, prcid);
            pre.setString(2, prtitle);
            pre.setDouble(3, prprice);
